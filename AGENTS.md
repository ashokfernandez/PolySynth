...
* **WHEN defining numeric types:** You MUST enforce strict type discipline for embedded portability. Use the `sample_t` alias for signals instead of hardcoded `double`. Use `float` for control parameters (e.g., spread, detune). Use `uint32_t` for timestamps to avoid expensive 64-bit atomic operations on ARM platforms. **Exception:** `SynthState` fields are `double` because the struct is shared with desktop and the aggregate-reset pattern (`*this = SynthState{}`) requires field-type consistency. The double→float conversion happens once per parameter update in Engine::UpdateState(), not per sample.
