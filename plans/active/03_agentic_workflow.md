...
```bash
# Build test/demo binaries
just build

# Run demo binary directly
./tests/build/demo_render_wav
...
1.  **Claim It**: Before starting, check `polysynth/plans/active/roadmap.md`. Find your Milestone.
...
## 6. Tooling Convention (Justfile First)

Use `just` for most workflows:

```bash
just                  # list available tasks
just check            # lint + test
just ci-pr            # lint + asan + tsan + test
just desktop-rebuild  # build + launch standalone app
just logs-latest      # inspect latest compact-run logs
```

If a task is not exposed via `just`, use scripts under `scripts/tasks/` (not root scripts).
