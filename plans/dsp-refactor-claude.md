...
The original DSP refactor plan (`plans/dsp_refactor.md`) was attempted on branch `dsp-refactor-sea` and failed. A single WIP commit changed ~40 files simultaneously, introduced DaisySP facades with broken PIMPL, changed every API signature, and never compiled. The branch has been abandoned.
